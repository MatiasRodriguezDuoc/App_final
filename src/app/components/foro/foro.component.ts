...
export class ForoComponent implements OnInit {
  ...
  crearPublicacion() {
    ...
    this.api.crearPublicacion(this.publicacion).toPromise().then(
      (publicacion) => this.mensajePublicacion('creada', publicacion.id)
    ).catch(
  ...
  actualizarPublicacion() {
    this.api.actualizarPublicacion(this.publicacion).toPromise().then(
      (publicacion) => this.mensajePublicacion('actualizada', publicacion.id)
    ).catch(
      (error) => showToast('El servicio API Rest de Publicaciones no está disponible')
    );
  }
  ...
  eliminarPublicacion(pub: any) {
    ...
    this.api.eliminarPublicacion(pub.id).toPromise().then(
      () => this.mensajePublicacion('eliminada', pub.id)
    ).catch(
      (error) => showToast('El servicio API Rest de Publicaciones no está disponible')
    );
  }
